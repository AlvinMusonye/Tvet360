...
const fetchFilteredStudentsForPrograms = async(token, progs, pageNumber, params) =>
{
    try{
      console.log(progs);
      console.log(params);
      let programCodes = [...progs.map(prog => prog.programCode)];
      console.log(programCodes);
      const response = await fetch(`${API_BASE_URL}/api/v1/student-enrollment/filt-student-dtl-any-prog?pageNumber=${pageNumber}&${new URLSearchParams(params).toString()}`, {
        headers: { "Content-Type": "application/json" /*'Authorization': `Bearer ${token}`*/ },
        method: "POST",
        body: JSON.stringify({"programCodes": [...programCodes]})
      });
...
export {
    ...
    fetchFilteredStudentsForPrograms
};